...
int i,addr; //i is used as a counter, addr is used to store the address in decimal

int main()
{
    f1=fopen("object.txt","r");  //Input file
    f2=fopen("output1.txt","w");    //Output file
    ...
    do{
        ...
        if (line[0] == 'T') { //If the line is a text record
            ...
            for (i = 9; line[i] != '\0'; i += 2) {
                fprintf(f2, "%04x\t\t\t\t%c%c\n", addr, line[i], line[i + 1]); //Printing the address and the object code
                addr++;
            }
        }
        else if (line[0] == 'H') {
